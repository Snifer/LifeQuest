# LifeQuest documentation

This folder contains the full user documentation for LifeQuest in English and Spanish.

## Quick path

1. New user: start with **Getting started**
2. Daily workflow: read **Daily notes**, **Quests and progression**, and **Weekly review**
3. Optional systems: read **Shop add-on** and **Health add-on**
4. If you use Kanban or project notes, check **Daily notes** for Markdown sync scope

## Language index

### English

- [Getting started](en/getting-started.md)
- [Quests and progression](en/quests-and-progression.md)
- [Daily notes](en/daily-notes.md)
- [Widgets](en/widgets.md)
- [Weekly review](en/weekly-review.md)
- [Shop add-on](en/shop-addon.md)
- [Health add-on](en/health-addon.md)
- [Troubleshooting](en/troubleshooting.md)
- [FAQ](en/faq.md)
- [Privacy and local-first behavior](en/privacy.md)

### Español

- [Primeros pasos](es/getting-started.md)
- [Quests y progresión](es/quests-and-progression.md)
- [Daily notes](es/daily-notes.md)
- [Widgets](es/widgets.md)
- [Revisión semanal](es/weekly-review.md)
- [Add-on de tienda](es/shop-addon.md)
- [Add-on de salud](es/health-addon.md)
- [Solución de problemas](es/troubleshooting.md)
- [Preguntas frecuentes](es/faq.md)
- [Privacidad y funcionamiento local-first](es/privacy.md)

## Notes

- The documentation describes the current plugin behavior.
- Optional add-ons remain optional.
- The plugin stores its data inside your vault by default.

## Maintainer notes

- [Subquests implementation phases](subquests.md)
